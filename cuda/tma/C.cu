...
// nvcc C.cu -arch=compute_90 -code=sm_90  -lcuda
...
constexpr uint32_t GMEM_HEIGHT = 64;

uint64_t size[rank] = {GMEM_WIDTH, GMEM_HEIGHT,100};

// The stride is the number of bytes to traverse from the first element of one row to the next.
// It must be a multiple of 16.
uint64_t stride[rank - 1] = {GMEM_WIDTH * sizeof(T), 128*128*2};

// The box_size is the size of the shared memory buffer that is used as the
// destination of a TMA transfer.
constexpr uint32_t SMEM_WIDTH = 128;
constexpr uint32_t SMEM_HEIGHT = 64;

uint32_t box_size[rank] = {SMEM_WIDTH, SMEM_HEIGHT,1};
...
__global__ void kernel(const __grid_constant__ CUtensorMap tensor_map) {
  ...
  if (thread0) {
    // 发起 TMA 二维异步拷贝操作
    cde::cp_async_bulk_tensor_3d_global_to_shared(&smem_buffer, &tensor_map, x, y, 0, bar);
  ...
  if (thread0) {
    cde::cp_async_bulk_tensor_3d_shared_to_global(&tensor_map, x, y, 0, &smem_buffer);
